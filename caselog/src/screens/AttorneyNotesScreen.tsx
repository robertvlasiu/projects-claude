...
export default function AttorneyNotesScreen({ navigation }: any) {
  ...
  async function handleSave() {
    if (!form.notes?.trim()) { Alert.alert('Required', 'Please add meeting notes.'); return; }
    setSaving(true);
    await add({ date: form.date ?? '', attorney_name: form.attorney_name ?? '', duration_minutes: form.duration_minutes ?? '', billed_hours: form.billed_hours ?? '', notes: form.notes ?? '', action_items: form.action_items ?? '', cost: form.cost ?? '', attachment_paths: form.attachment_paths ?? [] });
    setSaving(false);
    setModalOpen(false);
  ...
  return (
    <View style={styles.root}>
      ...
      />
      <FAB onPress={() => setModalOpen(true)} color="#f97316" />
